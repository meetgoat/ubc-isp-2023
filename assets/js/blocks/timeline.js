...
const Timeline = {
	...
	setupObserver() {
		...
		const downCallback = (entries, observer) => {
			entries.forEach((entry) => {
				if (entry.boundingClientRect.top < 81) {
					if (entry.isIntersecting) {
						console.log('test');
					} else {
						const timelineButton = jQuery('.isp-timeline__button[data-decade=' + jQuery( entry.target ).index() + ']');
		...
		const upCallback = (entries, observer) => {
			entries.forEach((entry) => {
				if (entry.boundingClientRect.top > 81 && entry.boundingClientRect.top < 500 ) {
					if (entry.isIntersecting) {
						const decade = jQuery(entry.target).closest('.isp-timeline__decade');
						const timelineButton = jQuery('.isp-timeline__button[data-decade=' + decade.index() + ']');
						Timeline.scrollToButton( timelineButton );
					}
				}
			});
